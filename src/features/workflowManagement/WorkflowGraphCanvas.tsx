 ...
 * - "+" on node when it has no outgoing transition → add connected next state (opens state sheet)
 * - "Add next state" outlined button for a free-standing state → page header beside view toggle (graph view).
 */
...
/** Outer hit target (px); inner dot stays visually small. */
const HANDLE_HIT = 28;
const HANDLE_DOT = 10;
...
// ─── Viewport center → flow coords (for placing new nodes in view) ───────────
...
// ─── Reciprocal edge pairs (A→B and B→A) for handle routing ───────────────────

function reciprocalPairKey(a: string, b: string): string {
  return a < b ? `${a}|||${b}` : `${b}|||${a}`;
}

function buildReciprocalStatePairSet(draft: WorkflowDraft): Set<string> {
  const out = new Set<string>();
  const hasEdge = (from: string, to: string) =>
    draft.states.some((s) => s.draftId === from && s.transitions.some((t) => t.targetDraftId === to));

  for (const s of draft.states) {
    for (const t of s.transitions) {
      if (s.draftId === t.targetDraftId) continue;
      if (hasEdge(t.targetDraftId, s.draftId)) {
        out.add(reciprocalPairKey(s.draftId, t.targetDraftId));
      }
    }
  }
  return out;
...
function handlesForDirectedEdge(
  ...
  reciprocalPairs: Set<string>,
): { sourceHandle: string; targetHandle: string } {
  if (sourceDraftId === targetDraftId) return { sourceHandle: H.sr, targetHandle: H.tl };
  if (!reciprocalPairs.has(reciprocalPairKey(sourceDraftId, targetDraftId))) {
...
function buildEdges(
  ...
  const reciprocalPairs = buildReciprocalStatePairSet(draft);
  ...
  for (const state of draft.states) {
    for (const t of state.transitions) {
      ...
      const { sourceHandle, targetHandle } = handlesForDirectedEdge(
        state.draftId,
        t.targetDraftId,
        reciprocalPairs,
      );

...
/** Sum of guards / async actions across all outgoing transitions (canvas chip totals). */
export function outgoingGuardsAndActionsTotals(state: WorkflowStateDraft): {
  guards: number;
  actions: number;
} {
  return state.transitions.reduce(
    (acc, t) => ({
      guards: acc.guards + t.guards.length,
      actions: acc.actions + t.actions.length,
    }),
    { guards: 0, actions: 0 },
  );
}

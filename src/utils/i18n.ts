...
export const STR = {
  ...
  explorations: {
    ...
    workflowsGlobalInstancesArchive: {
      navLabel: t(
        "explorations.workflowsGlobalInstancesArchive.navLabel",
        "Workflows (global instances archive)",
      ),
      pageTitle: t(
        "explorations.workflowsGlobalInstancesArchive.pageTitle",
        "Workflows — global instances layout (archive)",
      ),
      pageSubtitle: t(
        "explorations.workflowsGlobalInstancesArchive.pageSubtitle",
        "Frozen snapshot for discussion: one Active instances section below the template list (not scoped per template).",
      ),
      archiveAlert: t(
        "explorations.workflowsGlobalInstancesArchive.archiveAlert",
        "The active custom-attributes-v2 /workflows page nests instances under each template. This lab route keeps the earlier pattern for comparison.",
      ),
    },
  },
  ...
  workflowEditor: {
    ...
    addNextStageAriaLabel: t("workflowEditor.addNextStageAriaLabel", "Add next state"),
    ...
    /** Shown on workflow state cards — totals across outgoing transitions (matches design chips). */
    stateGuardsChipLabel: t("workflowEditor.stateGuardsChipLabel", "Guards:"),
    stateActionsChipLabel: t("workflowEditor.stateActionsChipLabel", "Actions:"),
    stateGuardsActionsCountNone: t("workflowEditor.stateGuardsActionsCountNone", "N/A"),
    stateGuardsChipAria: (count: number) =>
      t(
        "workflowEditor.stateGuardsChipAria",
        count > 0
          ? `${count} guard${count === 1 ? "" : "s"} on outgoing transitions`
          : "No guards on outgoing transitions",
      ),
    stateActionsChipAria: (count: number) =>
      t(
        "workflowEditor.stateActionsChipAria",
        count > 0
          ? `${count} async action${count === 1 ? "" : "s"} on outgoing transitions`
          : "No async actions on outgoing transitions",
      ),
    ...
    addNextStageButton: t("workflowEditor.addNextStageButton", "Add next state"),
    ...
    graphCanvasHint: t(
      ...
      "Drag nodes to rearrange. Draw a connection from one state to another to add a transition. Multiple arrows from one state mean branching — set triggers and guards on each transition. Delete an edge to remove a transition.",
    ),

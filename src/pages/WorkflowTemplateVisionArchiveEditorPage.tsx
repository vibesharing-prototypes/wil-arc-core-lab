...
import WorkflowTemplateCanvas from "../features/workflowManagementVisionArchive/WorkflowTemplateCanvas.js";
import WorkflowGraphCanvas from "../features/workflowManagementVisionArchive/WorkflowGraphCanvas.js";
import WorkflowStateSheet from "../features/workflowManagementVisionArchive/WorkflowStateSheet.js";
import WorkflowTransitionSheet from "../features/workflowManagementVisionArchive/WorkflowTransitionSheet.js";
import { useWorkflowTemplateEditor } from "../features/workflowManagementVisionArchive/useWorkflowTemplateEditor.js";
import type { CanvasItemSelection } from "../features/workflowManagementVisionArchive/WorkflowTemplateCanvas.js";
import type { WorkflowTemplate } from "../features/workflowManagementVisionArchive/types.js";
import type { StatusIndicatorColor, WorkflowDraft } from "../features/workflowManagementVisionArchive/draftTypes.js";
import type { Guard, Action } from "../features/workflowManagementVisionArchive/types.js";
 ...
 */
const ARCHIVE_WORKFLOWS_HOME = "/explorations/workflows-vision-archive";

const WorkflowTemplateVisionArchiveEditorPage: FC = () => {
  ...
  /** Set by `WorkflowGraphCanvas` when mounted — maps pane center to flow coordinates. */
  const viewportCenterFlowGetterRef = useRef<(() => { x: number; y: number } | null) | null>(null);

  /** Top-left `position` for a new node so its visual center sits at the viewport center (graph). */
  const getGraphPlacementForNewState = useCallback((): { x: number; y: number } | undefined => {
    ...
    return { x: center.x - 90, y: center.y - 45 };
  }, []);
  ...
  const handleAddNextStage = useCallback(
    (afterStateDraftId: string) => {
      const pos = canvasView === "graph" ? getGraphPlacementForNewState() : undefined;
      const newStateId = editor.addStateAfter(afterStateDraftId, pos);
      setPendingNewStateId(newStateId);
    },
    [editor, canvasView, getGraphPlacementForNewState],
  );
    ...
    <PageLayout>
      ...
      <Stack direction="row" alignItems="center" gap={0.5} sx={{ mb: 1, ml: -1 }}>
        <Button
          ...
          onClick={() =>
            navigate(ARCHIVE_WORKFLOWS_HOME, {
              state: {
...
export default WorkflowTemplateVisionArchiveEditorPage;
